...
class Word:
    ...
    def is_simple_literal(self):
        return len(self.parts) == 1 and isinstance(
            self.parts[0], (LiteralPart, SingleQuotedPart)
        )
...
class SimpleCommand:
    ...
    _array_idx_assigns: List[tuple] = field(default_factory=list)
    _inline_array_inits: List[tuple] = field(default_factory=list)
...
class IfNode:
    ...
    elifs: List[Tuple[List["Statement"], List["Statement"]]] = field(
...
class Tokenizer:
        ...
        while not self._at_end():
            ...
            if c == "2" and self._ch(1) == ">" and self._ch(2) == "&":
                ...
                while not self._at_end() and (
                    self._ch().isdigit() or self._ch() == "-"
                ):
                    fd.append(self._ch())
                    self._advance()
                self.tokens.append(
                    Token(TT.REDIR_FD, f"2>&{''.join(fd)}", line=self.line)
                )
                continue
            if (
                c == ">"
                and self._ch(1) == "&"
                and (self._ch(2).isdigit() or self._ch(2) == "-")
            ):
                ...
                while not self._at_end() and (
                    ...
                    self._advance()
                self.tokens.append(
                    Token(TT.REDIR_FD, f">&{''.join(fd)}", line=self.line)
                )
                continue
            ...
            if word:
                ...
                if word.is_simple_literal and raw in KEYWORDS:
                    self.tokens.append(
                        Token(KEYWORDS[raw], raw, word=word, line=self.line)
                    )
                else:
    ...
    def _read_word(self) -> Optional[Word]:
        ...
        has_glob = False
        # started = False
        in_quotes = False  # track if any part came from double quotes
        ...
        w = Word(parts=parts, has_glob=has_glob, quoted=in_quotes)
        return w
...
class Parser:
    ...
    def _expect(self, tt: TT, value=None) -> Token:
        ...
        if t.type != tt:
            raise ParseError(
                f"expected {tt.name} but got {t.type.name} '{t.value}'", t.line
            )
    ...
    def _and_or_list(self) -> Optional[Statement]:
        ...
        if not rest and len(pl.commands) == 1:
            ...
            if not pl.negated and isinstance(
                cmd,
                (
                    IfNode,
                    WhileNode,
                ...
                if getattr(cmd, "_inline_array_inits", []):
                    ...
                    if hasattr(cmd, "_array_idx_assigns"):
                        for arr_name, idx_expr, val_word in cmd._array_idx_assigns:
                            stmts.append(
                                ArrayIndexAssign(
                                    name=arr_name, index_expr=idx_expr, value=val_word
                                )
                            )
                    if len(stmts) == 1:
                        return stmts[0]
    ...
    def _command(self):
        ...
        if (
            t.type == TT.WORD
            and self._peek(1).type == TT.LPAREN
            and self._peek(2).type == TT.RPAREN
            and re.match(r"^[a-zA-Z_][a-zA-Z0-9_]*$", t.value)
        ):
    ...
    def _simple_command(self) -> Optional[SimpleCommand]:
            ...
            if "=" in raw and not raw.startswith("="):
                ...
                if re.match(r"^[a-zA-Z_][a-zA-Z0-9_]*$", name_part):
                    ...
                    if val_after_eq == "" and self._peek(1).type == TT.LPAREN:
                        ...
                        items = []
                        while (
                            self._cur().type != TT.RPAREN and self._cur().type != TT.EOF
                        ):
                            if self._cur().type == TT.NEWLINE:
                                self._advance()
                                continue
                ...
                # check for name+=value (string append)
                if name_part.endswith("+"):
                    base_name = name_part[:-1]
                    if re.match(r"^[a-zA-Z_][a-zA-Z0-9_]*$", base_name):
                        self._advance()
                        val_word = self._extract_value_after_eq(w)
                        # Use '+=name' prefix to signal append in codegen
                        assignments.append(("+=:" + base_name, val_word))
                        continue
            break
        ...
        if (
            ...
            and not array_idx_assigns
            and not inline_array_inits
            and not leading_array_inits
        ):
            return None

        cmd = SimpleCommand(
            args=args, redirections=redirections, assignments=assignments
        )
            ...
            if isinstance(part, (LiteralPart, SingleQuotedPart)):
                ...
                if not found_eq:
                    if char_count + len(text) > skip_name:
                        ...
                        if remainder:
                            new_parts.append(
                                LiteralPart(remainder)
                                if isinstance(part, LiteralPart)
                                else SingleQuotedPart(remainder)
                            )
                        found_eq = True
...
class CodeGen:
    def __init__(self):
        ...
        self.assoc_arrays: set = set()  # names declared with declare -A
    ...
    def generate(self, stmts: List[Statement]) -> str:
        ...
        if self.functions:
            header.append("")
            header.append("static FuncEntry _shellraiser_func_table[] = {{")
            for fd in self.functions:
                cname = self._c_func_name(fd.name)
                header.append(f'    {{ "{c_escape(fd.name)}", {cname} }},')
            header.append("};")
            header.append(
                f"static int _shellraiser_func_count = {len(self.functions)};"
            )
        else:
            header.append("")
        ...
        header.append(
            "    rt_register_functions(_shellraiser_func_table, _shellraiser_func_count);"
        )
        header.append("")
    ...
    def _gen_assignment(self, node: AssignmentNode):
        ...
        if node.name.startswith("+=:"):
            real_name = node.name[3:]
            tmp = self._tmp("_ap")
            self._emit(
                f'char *{tmp} = rt_concat(rt_get_var("{c_escape(real_name)}"), {val_expr});'
            )
            self._emit(f'rt_set_var("{c_escape(real_name)}", {tmp});')
    ...
    def _gen_array_list_assign(self, node: ArrayListAssign):
        ...
        self._emit(
            f'rt_array_set_list("{c_escape(node.name)}", {len(item_exprs)}, {arr_var});'
        )
    ...
    def _gen_array_index_assign(self, node: ArrayIndexAssign):
        ...
        if node.name in self.assoc_arrays:
            ...
            key_c = self._clean_assoc_key(node.index_expr)
            if needs_free:
                tmp = self._tmp()
                self._emit(f"char *{tmp} = (char *){val_expr};")
                self._emit(f'rt_assoc_set("{c_escape(node.name)}", {key_c}, {tmp});')
                self._emit(f"free({tmp});")
            else:
                self._emit(
                    f'rt_assoc_set("{c_escape(node.name)}", {key_c}, {val_expr});'
                )
        else:
            # Indexed array: index is arithmetic
            idx = c_escape(self._clean_arith_expr(node.index_expr))
            if needs_free:
                tmp = self._tmp()
                self._emit(f"char *{tmp} = (char *){val_expr};")
                self._emit(
                    f'rt_array_set("{c_escape(node.name)}", (int)rt_arith_eval("{idx}"), {tmp});'
                )
                self._emit(f"free({tmp});")
            else:
                self._emit(
                    f'rt_array_set("{c_escape(node.name)}", (int)rt_arith_eval("{idx}"), {val_expr});'
                )
    ...
    def _gen_inline_array_inits(self, inits):
        ...
        for arr_name, items in inits:
            ...
            if (
                ...
                self.indent += 1
                self._emit(
                    f'rt_array_append("{c_escape(arr_name)}", rt_get_arg({idx}));'
                )
                self.indent -= 1
                self.indent -= 1
                self._emit("}")
            ...
            if (
                ...
                if m:
                    ...
                    self._emit(
                        f'char **{arr} = rt_array_get_all("{c_escape(src_name)}", &{cnt});'
                    )
                    self._emit(f'rt_array_unset("{c_escape(arr_name)}");')
                    idx = self._tmp("_si")
                    ...
                    self.indent += 1
                    self._emit(
                        f'rt_array_append("{c_escape(arr_name)}", {arr}[{idx}]);'
                    )
                    self.indent -= 1
            ...
            if item_exprs:
                ...
                self._emit(
                    f'rt_array_set_list("{c_escape(arr_name)}", {len(item_exprs)}, {alv});'
                )
            else:
        ...
        if isinstance(node, Pipeline):
            ...
            for i, cmd in enumerate(node.commands):
                if isinstance(cmd, SimpleCommand):
                    ...
                    self._emit(
                        f'{pipe_var}[{i}] = (char *[]){{"echo", "compound-in-pipe-unsupported", NULL}};'
                    )

            self._emit(f"{pipe_var}[{len(node.commands)}] = NULL;")
            if node.background:
                self._emit(f"rt_exec_pipeline_bg({pipe_var}, {len(node.commands)});")
            else:
    ...
    def _gen_simple_command(self, cmd: SimpleCommand):
        ...
        for name, val in cmd.assignments:
            ...
            if name.startswith("+=:"):
                ...
                self._emit(f"free({tmp});")
                if needs_free:
                    # val_expr was also allocated - but it's been consumed by rt_concat
                    # rt_concat doesn't free its inputs, so we need to free the val too
                    pass  # val_expr is a const string or already in a tmp - handle below
                continue
        ...
        if hasattr(cmd, "_array_idx_assigns"):
            for arr_name, idx_expr, val_word in cmd._array_idx_assigns:
                ...
                if arr_name in self.assoc_arrays:
                    key_c = self._clean_assoc_key(idx_expr)
                    if needs_free:
                        tmp = self._tmp()
                        self._emit(f"char *{tmp} = (char *){val_expr};")
                        self._emit(
                            f'rt_assoc_set("{c_escape(arr_name)}", {key_c}, {tmp});'
                        )
                        self._emit(f"free({tmp});")
                    else:
                        self._emit(
                            f'rt_assoc_set("{c_escape(arr_name)}", {key_c}, {val_expr});'
                        )
                else:
                    ...
                    if needs_free:
                        ...
                        self._emit(f"char *{tmp} = (char *){val_expr};")
                        self._emit(
                            f'rt_array_set("{c_escape(arr_name)}", (int)rt_arith_eval("{idx}"), {tmp});'
                        )
                        self._emit(f"free({tmp});")
                    else:
                        self._emit(
                            f'rt_array_set("{c_escape(arr_name)}", (int)rt_arith_eval("{idx}"), {val_expr});'
                        )
        ...
        # Intercept 'declare -A name' to track associative arrays
        if first.is_simple_literal and first.literal_value in ("declare", "typeset"):
            for a in cmd.args[1:]:
                if a.is_simple_literal and a.literal_value == "-A":
                    continue
                if a.is_simple_literal and a.literal_value.startswith("-"):
                    continue
                # Extract name from 'name' or 'name=value'
                val = a.literal_value or ""
                aname = val.split("=")[0] if "=" in val else val
                if aname and any(
                    a2.is_simple_literal and a2.literal_value == "-A"
                    for a2 in cmd.args[1:]
                ):
                    self.assoc_arrays.add(aname)
        ...
        if bg:
            if has_redir:
                self._emit(
                    f"rt_exec_background_redir({argv_var}, {in_file}, {out_file}, {out_append}, {err_file}, {err_append});"
                )
            else:
            ...
            if has_redir:
                self._emit(
                    f"rt_exec_redir({argv_var}, {in_file}, {out_file}, {out_append}, {err_file}, {err_append});"
                )
            else:
                self._emit(f"rt_exec_simple({argv_var});")
    ...
    def _gen_argv_array(self, args: List[Word], prefix="_argv") -> tuple:
        ...
        Unquoted standalone variable args that expand to empty are skipped,
        matching bash word-splitting behavior."""
        # Check if any arg needs dynamic expansion or empty-skip
        has_dynamic = False
        for w in args:
            if len(w.parts) == 1 and isinstance(w.parts[0], VarPart):
                ...
                if (
                    vn in ("@", "*")
                    or re.match(r"^[a-zA-Z_]\w*\[[@*]\]$", vn)
                    or re.match(r"^![a-zA-Z_]\w*\[[@*]\]$", vn)
                ):
                    has_dynamic = True
                    break
                # Unquoted single var -> might be empty, needs dynamic skip
                if not w.quoted:
                    has_dynamic = True
                    break

        if not has_dynamic:
            # Static case: fixed number of args (all literals, quoted, or multi-part)
        ...
        # Dynamic case: some args expand to multiple entries or may be empty
        cap = self._tmp("_cap")
        cnt = self._tmp("_cnt")
        var = self._tmp(prefix)
        ...
        for w in args:
            if len(w.parts) == 1 and isinstance(w.parts[0], VarPart):
                ...
                if vn in ("@", "*"):
                    ...
                    self._emit(
                        f"    if ({cnt} + 1 >= {cap}) {{ {cap} *= 2; {var} = (char **)realloc({var}, sizeof(char *) * (size_t){cap}); }}"
                    )
                ...
                if m:
                    ...
                    if aname in self.assoc_arrays:
                        # Assoc array: expand values
                        # For "${assoc[@]}" we iterate values
                        self._emit(f"  char **{ae} = NULL;")
                        self._emit(
                            "  /* assoc values iteration not fully supported; use keys instead */"
                        )
                    else:
                        self._emit(
                            f'  char **{ae} = rt_array_get_all("{c_escape(aname)}", &{ac});'
                        )
                ...
                if m:
                    ...
                    self._emit(f"{{ int {ac} = 0;")
                    self._emit(
                        f'  char **{ae} = rt_assoc_keys("{c_escape(aname)}", &{ac});'
                    )
                    self._emit(f"  for (int {ai} = 0; {ai} < {ac}; {ai}++) {{")
                    self._emit(
                        f"    if ({cnt} + 1 >= {cap}) {{ {cap} *= 2; {var} = (char **)realloc({var}, sizeof(char *) * (size_t){cap}); }}"
                ...
                # Unquoted single variable: skip if empty (bash word-splitting)
                if not w.quoted:
                    access = self._gen_var_access(vn)
                    needs_free = self._var_access_needs_free(vn)
                    if needs_free:
                        tmp = self._tmp("_v")
                        self._emit(f"{{ char *{tmp} = {access};")
                        self._emit(f"  if ({tmp}[0]) {{")
                        self._emit(
                            f"    if ({cnt} + 1 >= {cap}) {{ {cap} *= 2; {var} = (char **)realloc({var}, sizeof(char *) * (size_t){cap}); }}"
                        )
                        self._emit(f"    {var}[{cnt}++] = {tmp};")
                        self._emit(f"  }} else free({tmp}); }}")
                    else:
                        tmp = self._tmp("_v")
                        self._emit(f"{{ const char *{tmp} = {access};")
                        self._emit(f"  if ({tmp}[0]) {{")
                        self._emit(
                            f"    if ({cnt} + 1 >= {cap}) {{ {cap} *= 2; {var} = (char **)realloc({var}, sizeof(char *) * (size_t){cap}); }}"
                        )
                        self._emit(f"    {var}[{cnt}++] = (char *){tmp};")
                        self._emit("  }} }}")
                    continue

            # Regular arg (literal, quoted, multi-part)
            expr = self._gen_word_expr(w)
            if self._word_needs_free(w):
                tmp = self._tmp("_a")
                self._emit(f"char *{tmp} = (char *){expr};")
    ...
    def _gen_for(self, node: ForNode):
        ...
        if node.items is None:
            ...
            self._emit(
                f"for (int {idx_var} = 1; {idx_var} <= {cnt_var}; {idx_var}++) {{"
            )
            self.indent += 1
            ...
            if not has_any_glob:
                # Check for array expansion items: ${arr[@]} or ${arr[*]} or ${!arr[@]}
                has_array_expand = False
                has_key_expand = False
                for w in node.items:
                    if len(w.parts) == 1 and isinstance(w.parts[0], VarPart):
                        vname = w.parts[0].name
                        if re.match(r"^[a-zA-Z_]\w*\[[@*]\]$", vname):
                ...
                if has_key_expand and len(node.items) == 1:
                    # Key iteration: for var in "${!assoc[@]}"
                    ...
                    self._emit(f"int {cnt} = 0;")
                    self._emit(
                        f'char **{keys} = rt_assoc_keys("{c_escape(aname)}", &{cnt});'
                    )
                    idx = self._tmp("_ki")
                    self._emit(f"for (int {idx} = 0; {idx} < {cnt}; {idx}++) {{")
                    self.indent += 1
                    self._emit(f'rt_set_var("{c_escape(node.var)}", {keys}[{idx}]);')
                    for s in node.body:
                        self._push_cleanup()
                        self._gen_statement(s)
                        self._pop_cleanup()
                    ...
                    self._emit("}")
                    self._emit(
                        f"for (int {idx} = 0; {idx} < {cnt}; {idx}++) free({keys}[{idx}]);"
                    )
                    self._emit(f"free({keys});")
                elif has_array_expand and len(node.items) == 1:
                    ...
                    self._emit(f"int {cnt} = 0;")
                    self._emit(
                        f'char **{arr} = rt_array_get_all("{c_escape(aname)}", &{cnt});'
                    )
                    idx = self._tmp("_ai")
                    self._emit(f"for (int {idx} = 0; {idx} < {cnt}; {idx}++) {{")
                    self.indent += 1
                    ...
                    if needs_split:
                        ...
                        self._emit(
                            f"const char *{items_var}[] = {{{', '.join(item_exprs)}}};"
                        )
                        idx = self._tmp("_i")
                        self._emit(
                            f"for (int {idx} = 0; {idx} < {len(item_exprs)}; {idx}++) {{"
                        )
                        self.indent += 1
                        self._emit(
                            f'rt_set_var("{c_escape(node.var)}", {items_var}[{idx}]);'
                        )
                        for s in node.body:
                            self._push_cleanup()
                            self._gen_statement(s)
                ...
                for w in node.items:
                    if w.has_glob and not w.quoted:
                        ...
                        self._emit(
                            f"char **{gl} = rt_glob_expand({pattern_expr}, &{gc});"
                        )
                        idx = self._tmp("_gi")
    ...
    def _gen_case(self, node: CaseNode):
        ...
        # ord_free = self._word_needs_free(node.word)
        ...
        for item in node.items:
            ...
            for pat in item.patterns:
                ...
                if pat_val == "*":
                    ...
                    conditions.append(
                        f'fnmatch("{c_escape(pat_val)}", {word_var}, 0) == 0'
                    )
                else:
    ...
    def _var_access_needs_free(self, name: str) -> bool:
        ...
        if name.startswith("#") and "[" not in name:
            return True
        # ${#arr[@]} / ${#arr[*]} - array/assoc length
        if name.startswith("#") and re.match(r"^#[a-zA-Z_]\w*\[[@*]\]$", name):
            return True
        # ${!arr[@]} - key iteration (assoc) - needs free
        if name.startswith("!") and re.match(r"^![a-zA-Z_]\w*\[[@*]\]$", name):
            return True
        # ${arr[@]} / ${arr[*]} - array/assoc join
        if re.match(r"^[a-zA-Z_]\w*\[[@*]\]$", name):
            return True
        # ${arr[idx]} - const pointer, no free (both indexed and assoc)
        if re.match(r"^[a-zA-Z_]\w*\[.+\]$", name):
            return False
        return False
    ...
    def _gen_cmd_subst_expr(self, cmd_str: str) -> str:
        ...
        if hs_match:
            ...
            if arr_m:
                ...
                self._emit(
                    f'char *{hs_var} = rt_array_join("{c_escape(aname)}", "\\n");'
                )
                result_var = self._tmp("_cr")
                self._emit(
                    f'char *{result_var} = rt_cmd_subst_stdin("{c_escape(cmd_part)}", {hs_var});'
                )
            ...
            if var_m:
                ...
                result_var = self._tmp("_cr")
                self._emit(
                    f'char *{result_var} = rt_cmd_subst_stdin("{c_escape(cmd_part)}", rt_get_var("{c_escape(vname)}"));'
                )
                return result_var
            ...
            result_var = self._tmp("_cr")
            self._emit(
                f'char *{result_var} = rt_cmd_subst_stdin("{c_escape(cmd_part)}", "{c_escape(hs_content)}");'
            )
            return result_var
    ...
    def _gen_var_access(self, name: str) -> str:
        ...
        if m:
            aname = m.group(1)
            if aname in self.assoc_arrays:
                return f'rt_itoa(rt_assoc_len("{c_escape(aname)}"))'
            return f'rt_itoa(rt_array_len("{c_escape(aname)}"))'

        # ${!arr[@]} / ${!arr[*]} - key iteration (assoc arrays)
        m = re.match(r"^!([a-zA-Z_]\w*)\[[@*]\]$", name)
        if m:
            aname = m.group(1)
            # Return joined keys as a space-separated string
            cnt = self._tmp("_kc")
            keys = self._tmp("_kv")
            self._emit(f"int {cnt} = 0;")
            self._emit(f'char **{keys} = rt_assoc_keys("{c_escape(aname)}", &{cnt});')
            bvar = self._tmp("_kb")
            self._emit(f"BStr {bvar} = bstr_new();")
            idx = self._tmp("_ki")
            self._emit(f"for (int {idx} = 0; {idx} < {cnt}; {idx}++) {{")
            self._emit(f'    if ({idx} > 0) bstr_append(&{bvar}, " ");')
            self._emit(f"    bstr_append(&{bvar}, {keys}[{idx}]);")
            self._emit(f"    free({keys}[{idx}]);")
            self._emit("}}")
            self._emit(f"free({keys});")
            result = self._tmp("_kr")
            self._emit(f"char *{result} = bstr_release(&{bvar});")
            return result
        ...
        if m:
            aname = m.group(1)
            if aname in self.assoc_arrays:
                return f'rt_assoc_join_values("{c_escape(aname)}", " ")'
        ...
        if m:
            ...
            key_expr = m.group(2)
            if aname in self.assoc_arrays:
                # Associative: key is a string (strip quotes, expand $var)
                clean_key = self._clean_assoc_key(key_expr)
                return f'rt_assoc_get("{c_escape(aname)}", {clean_key})'
            idx_expr = self._clean_arith_expr(key_expr)
            return f'rt_array_get("{c_escape(aname)}", (int)rt_arith_eval("{c_escape(idx_expr)}"))'

        # plain variable
        return f'rt_get_var("{c_escape(name)}")'

    def _clean_assoc_key(self, key_expr: str) -> str:
        """Generate a C expression for an associative array key.
        Handles quoted strings and $var references."""
        key = key_expr.strip()
        ...
        if m:
            return f'rt_get_var("{c_escape(m.group(1))}")'
        # Literal key
        return f'"{c_escape(key)}"'
...
def main():
    ...
    p.add_argument(
        "--cflags", default=None, help='Custom compiler flags (default: "-O3 -Wall")'
    )
    p.add_argument(
        "--emit-only",
        action="store_true",
    ...
    for path, label in [(runtime_c, "bash_runtime.c"), (runtime_h, "bash_runtime.h")]:
        if not os.path.isfile(path):
            print(
                f"shellraiser: error: runtime file '{label}' not found at {path}\n"
    ...
    if args.output:
        ...
        out_bin = os.path.join(
            os.path.dirname(os.path.abspath(args.script)), name_no_ext
        )
    ...
    try:
        ...
        if cc_resolved is None:
            print(
                f"shellraiser: error: compiler '{cc}' not found on PATH",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if result.returncode != 0:
            ...
            print(
                f"shellraiser: compilation failed (exit {result.returncode})",
                file=sys.stderr,
            )
            sys.exit(1)
