...
class CodeGen:
    ...
    def generate(self, stmts: List[Statement]) -> str:
        ...
        # --source mode: run script, then dump shell-evaluable output
        header.append('    /* --source mode: run script body, then output shell commands */')
        header.append('    int _source_mode = (argc >= 2 && strcmp(argv[1], "--source") == 0);')
        header.append('    int _saved_stdout = -1;')
        header.append('    if (_source_mode) {')
        header.append('        /* Redirect script stdout to stderr so eval only captures source output */')
        header.append('        _saved_stdout = dup(STDOUT_FILENO);')
        header.append('        dup2(STDERR_FILENO, STDOUT_FILENO);')
        header.append('    }')
        header.append('')
        ...
        footer = [
            '',
            '    if (_source_mode) {',
            '        /* Restore stdout and dump source output */',
            '        dup2(_saved_stdout, STDOUT_FILENO);',
            '        close(_saved_stdout);',
            '        rt_dump_source_output();',
            '    }',
            '',
            '    rt_cleanup();',
            '    return rt.last_exit;',
    ...
    def _gen_assignment(self, node: AssignmentNode):
        ...
        if node.name.startswith('+=:'):
            ...
            if needs_free:
                ...
                self._emit(f'rt_append_var("{c_escape(real_name)}", {tmp});')
                self._emit(f'free({tmp});')
            else:
                self._emit(f'rt_append_var("{c_escape(real_name)}", {val_expr});')
            return
    ...
    def _gen_array_index_assign(self, node: ArrayIndexAssign):
        ...
        if node.name in self.assoc_arrays:
            ...
            if needs_free:
                ...
                self._emit(f'rt_assoc_set("{c_escape(node.name)}", {key_c}, {val_expr});')
        else:
    ...
    def _gen_simple_command(self, cmd: SimpleCommand):
            ...
            for arr_name, idx_expr, val_word in cmd._array_idx_assigns:
                ...
                if arr_name in self.assoc_arrays:
                    ...
                    if needs_free:
                        tmp = self._tmp()
                        self._emit(f'char *{tmp} = (char *){val_expr};')
                        self._emit(f'rt_assoc_set("{c_escape(arr_name)}", {key_c}, {tmp});')
                        self._emit(f'free({tmp});')
                    else:
                        self._emit(f'rt_assoc_set("{c_escape(arr_name)}", {key_c}, {val_expr});')
                else:
        ...
        if first.is_simple_literal and first.literal_value in ('declare', 'typeset'):
            for a in cmd.args[1:]:
                ...
                if aname and any(a2.is_simple_literal and a2.literal_value == '-A' for a2 in cmd.args[1:]):
    ...
    def _gen_argv_array(self, args: List[Word], prefix='_argv') -> tuple:
        ...
        for w in args:
            if len(w.parts) == 1 and isinstance(w.parts[0], VarPart):
                vn = w.parts[0].name
                if vn in ('@', '*') or re.match(r'^[a-zA-Z_]\w*\[[@*]\]$', vn) or re.match(r'^![a-zA-Z_]\w*\[[@*]\]$', vn):
                    has_dynamic = True
                    break
            ...
            if len(w.parts) == 1 and isinstance(w.parts[0], VarPart):
                ...
                if m:
                    ...
                    if aname in self.assoc_arrays:
                        ...
                        self._emit(f'  /* assoc values iteration not fully supported; use keys instead */')
                    else:
                ...
                if m:
                    ...
                    ai = self._tmp('_ai')
                    self._emit(f'{{ int {ac} = 0;')
                    self._emit(f'  char **{ae} = rt_assoc_keys("{c_escape(aname)}", &{ac});')
                    self._emit(f'  for (int {ai} = 0; {ai} < {ac}; {ai}++) {{')
                    self._emit(f'    if ({cnt} + 1 >= {cap}) {{ {cap} *= 2; {var} = (char **)realloc({var}, sizeof(char *) * (size_t){cap}); }}')
                    self._emit(f'    {var}[{cnt}++] = {ae}[{ai}];')
                    self._emit(f'  }}')
        ...
        if node.items is None:
            ...
            if not has_any_glob:
                ...
                if has_key_expand and len(node.items) == 1:
                    ...
                    self.indent -= 1
                    self._emit('}')
                    self._emit(f'for (int {idx} = 0; {idx} < {cnt}; {idx}++) free({keys}[{idx}]);')
...
def main():
    ...
    p.add_argument('--sourceable', action='store_true',
                   help='Produce a polyglot that can be both sourced and executed')
    ...
    try:
        ...
        # -- Optionally wrap in a sourceable polyglot --
        if args.sourceable:
            ...
            binary_hash = hashlib.md5(binary_data).hexdigest()[:12]
            b64_payload = base64.b64encode(binary_data).decode('ascii')
            b64_lines = [b64_payload[i:i+76] for i in range(0, len(b64_payload), 76)]
            b64_text = '\n'.join(b64_lines)

            script_name = os.path.basename(out_bin)
            header = f'''#!/bin/bash
# shellraiser polyglot -- source this OR execute it.
#   source ./{script_name}    ->   loads compiled functions + variables
#   ./{script_name} [args]    ->   runs the compiled script
#
# The compiled binary is base64-encoded below and cached on first run.
_SHELLRAISER_CACHE="${{SHELLRAISER_CACHE:-${{HOME}}/.cache/shellraiser}}"
_SHELLRAISER_BIN="${{_SHELLRAISER_CACHE}}/{binary_hash}"
if [ ! -x "$_SHELLRAISER_BIN" ]; then
    mkdir -p "$_SHELLRAISER_CACHE" 2>/dev/null
    base64 -d << '__SHELLRAISER_PAYLOAD__' > "$_SHELLRAISER_BIN"
{b64_text}
__SHELLRAISER_PAYLOAD__
    chmod +x "$_SHELLRAISER_BIN" 2>/dev/null
fi
if [ -n "${{BASH_SOURCE[0]}}" ] && [ "${{BASH_SOURCE[0]}}" != "$0" ]; then
    eval "$("$_SHELLRAISER_BIN" --source "$@")"
    return 2>/dev/null
fi
exec "$_SHELLRAISER_BIN" "$@"
'''
            # Overwrite the binary with the polyglot
            with open(out_bin, 'w') as f:
                f.write(header)
            os.chmod(out_bin, 0o755)

            polyglot_kb = len(header) // 1024
            print(f"shellraiser: sourceable ({polyglot_kb}KB, "
                  f"source or execute)", file=sys.stderr)
